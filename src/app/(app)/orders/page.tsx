...
export default function OrdersPage() {
  ...
  const { orders, total, mutate } = useOrders({ status: statusTab, search, page, limit: LIMIT });
    ...
    <div>
      <PageHeader title="Orders" description="Manage and track customer orders">
        <Link href="/orders/new">
          <Button className="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold h-9 px-4 rounded-lg shadow-sm">
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        {orders.length === 0 ? (
          <EmptyState icon={ShoppingCart} title="No orders found" description="Create your first order or adjust the filters.">
            <Link href="/orders/new">
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-slate-50/80 border-b border-slate-200">
                  <TableHead className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Order #</TableHead>
                  <TableHead className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Customer</TableHead>
                  <TableHead className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Items</TableHead>
              ...
              <TableBody>
                {orders.map((o) => (
                  <TableRow key={o.id} className="hover:bg-slate-50/60 transition-colors">
                    <TableCell>
                      <span className="font-mono text-sm font-bold text-indigo-600">{o.orderNumber}</span>
                    </TableCell>
                    <TableCell className="font-medium text-slate-800">{o.customerName}</TableCell>
                    <TableCell className="text-slate-500 text-sm">{o.items.length} item{o.items.length !== 1 ? 's' : ''}</TableCell>
                    <TableCell className="font-bold text-slate-800">${o.totalPrice.toFixed(2)}</TableCell>
                    <TableCell><OrderStatusBadge status={o.status as OrderStatus} /></TableCell>
                    <TableCell className="text-slate-400 text-xs">{formatDateTime(o.createdAt)}</TableCell>
